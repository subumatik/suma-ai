...
interface AvukatlarClientProps {
  lawyers: any[];
  userId: string;
}

export default function AvukatlarClient({ lawyers, userId }: AvukatlarClientProps) {
            ...
            <Card sx={{ height: '100%' }}>
              <CardContent sx={{ p: 3 }}>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  <Button variant="contained" fullWidth startIcon={<CalendarMonth />} onClick={() => router.push(`/randevular?lawyer=${l.id}`)}>
                    Randevu Al
                  </Button>
                  <Button variant="outlined" fullWidth startIcon={<Person />} onClick={() => router.push(`/mesajlar?u=${l.id}`)}>
                    Mesaj
                  </Button>
                </Box>
              </CardContent>
