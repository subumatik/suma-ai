...
export default function DosyalarClient({ dosyalar, role, userId, categories, statuses, clients }: DosyalarClientProps) {
      ...
      <Grid container spacing={2}>
        {filtered.map((d) => (
          <Grid size={{ xs: 12, md: 6, lg: 4 }} key={d.id}>
            <Card sx={{ cursor: 'pointer', height: '100%' }} onClick={() => router.push(`/dosyalar/${d.id}`)}>
              <CardContent sx={{ p: 3 }}>
