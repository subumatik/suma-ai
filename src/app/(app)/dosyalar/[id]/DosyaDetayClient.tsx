...
export default function DosyaDetayClient({ dosya, documents, statusUpdates, messages, statuses, categories, role, userId }: Props) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filePath = `${dosya.id}/${Date.now()}_${file.name}`;
    ...
    await supabase.from('dosya_documents').insert({
      dosya_id: dosya.id,
      file_name: file.name,
  ...
  return (
    <Box>
      <Button onClick={() => router.push('/dosyalar')} sx={{ mb: 2 }}>← Dosyalara Dön</Button>
              ...
              <List>
                {documents.map((d) => (
                  <ListItem key={d.id} sx={{ bgcolor: 'action.hover', borderRadius: 2, mb: 1 }}>
                    ...
                    />
                    <IconButton href={d.file_url} target="_blank" rel="noopener noreferrer">
                      <Download />
                    </IconButton>
                  </ListItem>
                ))}
